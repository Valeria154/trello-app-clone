...
const containerTodoElement = document.querySelector('#todoContainer')
...
function handleSubmitForm(event) {
	...
	// const { editedId } = formElement.dataset; // Для редактирования формы
	// if (editedId) {
	// 	// Найти и обновить существующую задачу
	...
	// 	render(todos); // Здесь вы можете обновлять конкретную карточку вместо всего списка
	// } else {
	// 	// Создать новую задачу
	// 	const newTodo = new Todo(formDataObject);
	// 	todos.push(newTodo);
	// 	setDataToStorage(todos);
	// 	cardContainer.insertAdjacentHTML('beforeend', buildTodoTemplate(newTodo)); // Добавляем только новую карточку
	// }
...
// 	const { editedId } = formElement.dataset //для редактирование формы
// 	if (editedId) {
// 		const editedTodoIndex = todos.findIndex(todo => todo.id == editedId)
// 		todos[editedTodoIndex] = { ...todos[editedTodoIndex], ...formDataObject }
// 		setDataToStorage(todos)
...
function buildTodoTemplate({ id, title, description, user, status, createdAt }) {
	const data = currentDate(createdAt) // Преобразуем дату
	return `
		 <div data-id="${id}" class="card bg-white">
			  <div class="d-flex justify-content-center" role="group">
					<button type="button" class="btn btn-light" data-role="edit">EDIT</button>
