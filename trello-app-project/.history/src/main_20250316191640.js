...
function render() {
	...
	todos.forEach(todo => {
		const newTodo = buildTodoTemplate(todo)
		todoContainerElement.insertAdjacentHTML('afterbegin', newTodo) //новые задачи будут отображаться сверху так как afterbegin вставляет новую задачу в начало контейнера
	})
}
